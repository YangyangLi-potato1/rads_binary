...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.fuse = nn.Conv3d(in_channels=2, out_channels=1, kernel_size=1)
    ...
    def forward(self, x_ct: torch.Tensor,x_bw:torch.Tensor) -> torch.Tensor:
        # x=torch.cat([x_ct,x_bw],dim=1)
        # x=self.fuse(x)
        x=x_bw

        x1 = self.features(x) #[2,1024,15,11,3]
        x = self.class_layers(x1) #[2,2] probability
        return x,x1
        # return x
